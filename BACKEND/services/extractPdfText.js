...
export const extractPdfText = async (filePath) => {
  ...
  if (!rawText || rawText.length < 20) {
    throw new Error(
      "Unable to extract text. Please upload a text-based PDF (not scanned)."
    );
  }
  ...
  console.log(
    `Extracted PDF text: ${structuredText.length} chars total | sending ${aiText.length} chars to AI`
  );
